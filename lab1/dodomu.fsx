...
let distanceBetweenImageAndTrainingData unknown = trainRecords |> Array.map(fun u -> {Label=u.label; ImagesDistance=distance unknown u.pixels})

let classify k_neighboors unknown=
 let result =
  distanceBetweenImageAndTrainingData unknown
...
classify k valRecords.[0].pixels
...
// [ YOUR CODE GOES HERE! ]
let evaluate k validationSet = 
 let result =
  validationSet
  |> Array.sumBy(fun x -> if x.label = classify k x.pixels then 1 else 0)
 (float)result / (float)validationSet.Length

let evaluate_knn k = evaluate k valRecords

for i=1 to 5 do
 let precision =  evaluate_knn i
