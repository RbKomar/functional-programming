...
// [ YOUR CODE GOES HERE! ]
#load "Helper.fsx"
//prettyPrint trainRecords.[1].pixels
...
let distdance (p1: int[]) (p2: int[]) = 42
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/// Ponizej znajduje się implementacja brute-force klasyfikatora knn oraz przykładowe zastosowanie dla k w zakresie od 1 do 5 oraz dla 3 różnych
/// miar odległosci, kolejno: Manhattan, Euclidean oraz Cambera dla odpowiednio pomniejszonego zbioru Cifar-10
/// Autorem kodu jest Robert Komar 173237
  
open System
open System.IO

let trainSamples = File.ReadAllLines("trainingsample.csv")
let valSamples = File.ReadAllLines("validationsample.csv")

let splitTrainSamples = trainSamples |> Array.map( fun s-> s.Split(','))
let splitValSamples = valSamples |> Array.map( fun s-> s.Split(','))

let noHeaderTrainSamples = splitTrainSamples.[1 ..]
let noHeaderValSamples = splitValSamples.[1 ..] 

let toIntegerTrainSamples = noHeaderTrainSamples |> Array.map(fun s-> Array.map(fun c -> (int)c) s)
let toIntegerValSamples = noHeaderValSamples |> Array.map(fun s-> Array.map(fun c -> (int)c) s)

type DatasetRecords = {label:int; pixels:int[]}
let trainRecords = toIntegerTrainSamples |> Array.map(fun r -> {label= r.[0]; pixels= r.[1 ..] })
let valRecords = toIntegerValSamples |> Array.map(fun r -> {label= r.[0]; pixels= r.[1 ..] })

// obliczenie normy L2 
let ManhattanDistance (P1: int[]) (P2: int[]) =
 Array.map2(fun p1 p2 -> abs(p1 - p2)) P1 P2

let EuclideanDistance (P1: int[]) (P2: int[]) =
 Array.map2(fun p1 p2 -> (p1 - p2)*(p1 - p2)) P1 P2

let CamberaDistance (P1: int[]) (P2: int[]) = 
 Array.map2(fun p1 p2 -> (int)(abs((float)(p1 - p2)/(float)(p1 + p2)))) P1 P2

let distance (P1: int[]) (P2: int[]) norm = 
 norm P1 P2
 |> Array.sumBy(fun p-> (float)p)
 |> sqrt


type ResultRecords = {Label:int; ImagesDistance: float}
let distanceBetweenImageAndTrainingData unknown norm = trainRecords |> Array.map(fun u -> {Label=u.label; ImagesDistance=distance unknown u.pixels norm})

let classify k_neighboors norm unknown =
 let result =
  distanceBetweenImageAndTrainingData unknown norm
  |> Array.sortBy(fun x -> x.ImagesDistance)
  
 result.[.. k_neighboors-1] 
 |> Array.countBy(fun x-> x.Label) //zliczenie klas wystepujacych w nablizszej odleglosci dla przewidywanej wartosci 
 |> Array.maxBy snd //wybranie takiej krotki która zawiera najwieksza druga wartosc
 |> fst // wybranie pierwszego elementu krotki


let evaluate k norm validationSet  = 
 let result =
  validationSet
  |> Array.sumBy(fun x -> if x.label = classify k norm x.pixels  then 1 else 0)
 (float)result / (float)validationSet.Length

// Ewaluacja dla różnych norm oraz różnych wartości k z przekazywaniem funkcji obliczania normy jako parametr
let evaluateKnnEuclidean k = evaluate k EuclideanDistance valRecords  //zastowowanie closure
printf "Odległość Euklidesowa"
for i=1 to 5 do
 let precision =  evaluateKnnEuclidean i
 printfn "Results of classification for %d neighboors =  %f" i precision

let evaluateKnnManhattan k = evaluate k ManhattanDistance valRecords  
printf "Odległość Manhattan"
for i=1 to 5 do
 let precision =  evaluateKnnManhattan i
 printfn "Results of classification for %d neighboors =  %f" i precision

let evaluateKnnCambera k = evaluate k CamberaDistance valRecords  
printf "Odległość Cambera"
for i=1 to 5 do
 let precision =  evaluateKnnCambera i
 printfn "Results of classification for %d neighboors =  %f" i precision

// zmieniając wartość k widać, że jakość predykcji maleje dla odległości euklidesowej, a rośnie dla odległości manhattan, aż do 3k 
// Dla odległości cambera otrzymane wyniki są wyjątkowo niskie, może być to spowodowane rzutowaniem dystansu na int albo dystans ten
// jest zwyczajnie nieprzydatny w tego typu klasyfikacji obrazów